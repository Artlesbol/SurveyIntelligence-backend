...
def send_email_confirm(email, code):
    ...
    text_content = '''感谢注册问卷智汇，这里是问卷智汇，专注于管理与发布问卷！\
                    如果你看到这条消息，说明你的邮箱服务器不提供HTML链接功能，请联系管理员！'''

    html_content = '''
                    <p>感谢注册<a href="{}/confirm?code={}" target=blank>问卷智汇</a>，\
...
def send_email_change_confirm(email, code):
    ...
    text_content = '''感谢使用问卷智汇，这里是问卷智汇，专注于管理与发布问卷！\
                    您的邮箱验证码为 {}\
                    验证码有效期为{}天'''.format(code, settings.CONFIRM_DAYS)

    html_content = '''
                    <p>感谢使用<a href="{}" target=blank>问卷智汇</a>，\
                    这里是问卷智汇，专注于管理与发布问卷！</p>
                    <p>您的邮箱验证码为 {}</p>
                    <p>验证码有效期为{}天！</p>
                    '''.format(settings.WEB_FRONT, code, settings.CONFIRM_DAYS)   # url must be corrected
